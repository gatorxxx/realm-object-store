...
struct SyncSession::State {
    ...
    virtual void enter_state(std::unique_lock<std::mutex>&, SyncSession&, bool is_client_resync = false) const { }
...
struct sync_session_states::Inactive : public SyncSession::State {
    void enter_state(std::unique_lock<std::mutex>& lock, SyncSession& session, bool is_client_reset) const override
    {
        ...
        std::error_code ec = util::error::operation_aborted;
        if (is_client_reset)
            ec = sync::ProtocolError::bad_client_file;
