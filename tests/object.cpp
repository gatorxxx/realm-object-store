...
TEST_CASE("Embedded Object") {
    ...
    SECTION("set_property_value() on list of embedded objects") {
        ...
        SECTION("throws when given a managed object") {
            realm->begin_transaction();
            REQUIRE_THROWS_WITH(obj.set_property_value(ctx, "array", util::Any{AnyVector{list.get(0)}},
                                                       CreatePolicy::UpdateModified),
                                "Setting a List entry to an an existing managed embedded object is not supported.");
            realm->cancel_transaction();
        }
